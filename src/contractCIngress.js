...
/**
 * Policy-independent exact Contract C ingress for Decision Engine policies.
 *
 * Semantic policy code must not inspect Contract C until this boundary has
 * established the external whole-object digest, exact released authority,
 * canonical Contract C validity, and exact expected Contract B binding.
 */
export function loadExactContractCForDecision({
  contractCBytes,
  expectedContractCSha256,
  contractCAuthorityRoot,
  expectedContractB,
...
export function contractCInputAuthority(contractC, exactContractCSha256) {
  return {
    kind: "contract-c",
