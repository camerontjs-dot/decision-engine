...
import { decideParentBoundPolicy } from "../src/parentBoundPolicyDispatch.js";

const USAGE = `Usage:
  node scripts/decision-engine-parent-bound-policy-evaluate.mjs \\
    --contract-c <path> \\
    --contract-c-sha256 <sha256:...> \\
    --consumer-authority <exact frozen independent-consumer checkout> \\
    --consumer-inputs <JSON path> \\
    --policy-id <maintained policy id> \\
    --policy-version <maintained policy version> \\
    --target <JSON path> \\
    [--python <python executable>]

The command emits the native Decision object as JSON on stdout.
It does not accept an effect or requested operation.
It does not canonicalize against released Contract D, authorize, or execute.
`;
...
try {
  ...
  if (parsed.help) {
    ...
    const decision = decideParentBoundPolicy({
      ...
      decisionContext: {
        policy: {
          id: values.get("--policy-id"),
          version: values.get("--policy-version"),
        },
        target,
      },
      pythonExecutable,
