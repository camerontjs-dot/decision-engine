...
import { evaluateContractC2Decision } from "../src/contractC2DecisionRuntime.js";
...
const USAGE = `Usage:
  node scripts/decision-engine-evaluate-c2.mjs \\
    ...
    --contract-c2-authority <exact Contract C 2.0.0 candidate checkout> \\
...
The command emits only canonical Contract D 1.0.0 JSON bytes on stdout when successful.
It performs no Authorization, execution, network mutation, or downstream action.
Contract C2 ingress is strict: exact head b42c827acb0a9fe65353354d709add0e27bab307,
version 2.0.0, wire profile contract-c-successor-candidate-a-rc2-research.
Strict Contract C 1.0 ingress continues to reject C2 objects; no downgrade exists.
`;
...
try {
  ...
  if (parsed.help) {
    ...
    const decision = evaluateContractC2Decision({
      contractC2Bytes: readBytes(values.get("--contract-c2"), "cannot read Contract C2 bytes"),
      expectedContractC2Sha256: values.get("--contract-c2-sha256"),
      contractC2AuthorityRoot: values.get("--contract-c2-authority"),
      expectedContractB,
      decisionContext,
      pythonExecutable,
    });
